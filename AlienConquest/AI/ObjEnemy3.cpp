...
CTexture **CObjEnemy3::m_regular = new CTexture *[3];
CTexture **CObjEnemy3::m_firing = new CTexture *[9];
CTexture **CObjEnemy3::m_jetting = new CTexture *[12];
CTexture **CObjEnemy3::m_hit = new CTexture *[2];
RECT CObjEnemy3::m_myboundrects[4];
const char *CObjEnemy3::m_grdir = "enemies/enemy3";
...
CObjEnemy3::CObjEnemy3(void)
{
	...
	m_fire_time.setInterval(600+rand()%900);
...
void CObjEnemy3::Jet()
{
	if (m_state == REGULAR) {
		m_state = JETTING;
		if (CWorld::IsOnScreen((int)m_dpos_x,(int)m_dpos_y,156,118)) g_SndMgr->play(ENEMY3_JET,0);
		m_jet_seq.Reset();
...
void CObjEnemy3::Fire()
{
	if (m_state != FIRING) {
		...
		if (CWorld::IsOnScreen((int)m_dpos_x,(int)m_dpos_y,156,118)) g_SndMgr->play(ENEMY3_FIRE,0);
	}
}
...
void CObjEnemy3::_LoadGraphics()
{
   ...
   for (int x=0; x < 2; x++) {
	...
	SetRect((LPRECT)&m_myboundrects[0],42,17,71,87);
	SetRect((LPRECT)&m_myboundrects[1],76,35,114,52);
	SetRect((LPRECT)&m_myboundrects[2],4,87,79,106);
	SetRect((LPRECT)&m_myboundrects[3],79,85,109,96);
...
void CObjEnemy3::_Behaviour(void)
{
   ...
   case RANDOM:
	   if (m_rand_time.CmpTime()) {
		   ...
		   m_fire_time.setInterval(600+rand()%600);
	   }
	   break;
   case EXIT_STAGE_LEFT:
	   ...
	   break;
   case STAYANDFIRE:
	   m_fire_time.setInterval(600+rand()%400);
...
void CObjEnemy3::Collision(CObj *colwith)
{
	if (++m_curdamage >= m_numdamage) {
	   if (CWorld::IsOnScreen((int)m_dpos_x,(int)m_dpos_y,156,118)) g_SndMgr->play(ENEMY3_DEAD,0);
		if (rand()%500 > 150) {
			CObjPowerUp *p = new CObjPowerUp;
			//p->ChangePowerup();
			p->SetPosition(m_dpos_x+30,m_dpos_y+20);
			g_ObjMgr->add(p);
		}
	   m_state=DEAD;
	   CObj::Collision(colwith);
		...
		if (CWorld::IsOnScreen((int)m_dpos_x,(int)m_dpos_y,156,118)) g_SndMgr->play(ENEMY3_HIT,0);
		m_state = HIT;
		m_hit_seq.Reset();
	}
}
